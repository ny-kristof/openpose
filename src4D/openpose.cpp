...
//ebbe kell eltárolni az OpenPose-ból kinyert pose-t 
...
// a "type" skeleton type-ot átalakítja "tarType" típusúvá
...
//detection-ök fileból való beolvasása
std::vector<OpenposeDetection> ParseDetections(const std::string& filename)
{
	...
	for (OpenposeDetection& detection : detections) {
		for (int jIdx = 0; jIdx < def.jointSize; jIdx++) {
			...
			//std::cout << "detection.hoints"<< jIdx << ": " <<"\t" << detection.joints[jIdx] << std::endl;
 		}
		for (int pafIdx = 0; pafIdx < def.pafSize; pafIdx++) {
			const int jAIdx = def.pafDict(0, pafIdx);
			const int jBIdx = def.pafDict(1, pafIdx);
			...
			std::cout << "detection.pafz" << pafIdx << " of frame" << framec << " , between joints " << jAIdx << " and " << jBIdx << ": " << std::endl << detection.pafs[pafIdx] << std::endl;
		}
		framec++;
	}
	fs.close();
